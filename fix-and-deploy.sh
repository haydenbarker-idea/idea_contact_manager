...
# Contact Exchange - Fix Nginx and Complete Deployment
# Run with: sudo bash fix-and-deploy.sh
...
echo "================================================"
echo "Contact Exchange - Fix and Deploy"
echo "================================================"
echo ""
...
echo -e "${GREEN}Step 1: Fixing Nginx configuration...${NC}"
...
echo -e "${GREEN}Step 5: Obtaining SSL certificate...${NC}"
certbot --nginx -d $DOMAIN --non-interactive --agree-tos -m $EMAIL --redirect

if [ $? -eq 0 ]; then
    echo -e "${GREEN}✓ SSL certificate obtained${NC}"
else
    echo -e "${YELLOW}⚠ SSL certificate issue (may need DNS propagation)${NC}"
fi

echo -e "${GREEN}Step 6: Setting up auto-renewal for SSL...${NC}"
...
echo -e "${GREEN}Step 8: Cloning repository from GitHub...${NC}"
cd /var/www/contact-exchange
git clone $REPO_URL .
echo -e "${GREEN}✓ Repository cloned${NC}"

echo -e "${GREEN}Step 9: Prompting for your contact information...${NC}"
echo ""
...
echo -e "${GREEN}Step 11: Installing dependencies...${NC}"
cd /var/www/contact-exchange
npm install

echo -e "${GREEN}Step 12: Setting up database...${NC}"
npx prisma generate
npx prisma db push

echo -e "${GREEN}Step 13: Building application...${NC}"
npm run build

echo -e "${GREEN}Step 14: Setting permissions...${NC}"
chown -R root:root /var/www/contact-exchange

echo -e "${GREEN}Step 15: Starting application...${NC}"
...
if certbot certificates 2>&1 | grep -q "$DOMAIN"; then
    ...
    echo -e "  ${YELLOW}⚠ SSL certificate not found (may need DNS propagation)${NC}"
fi
...
if curl -sf -k https://$DOMAIN > /dev/null 2>&1; then
    ...
    echo -e "  ${YELLOW}⚠ Site not accessible via HTTPS (check DNS/firewall)${NC}"
fi
...
journalctl -u contact-exchange -n 10 --no-pager
echo ""
echo "================================================"
echo ""
...
if [ "$VALIDATION_PASSED" = true ]; then
    ...
    echo "1. Check logs: journalctl -u contact-exchange -f"
    echo "2. Check Nginx: systemctl status nginx"
    echo "3. Check app: curl http://localhost:3000"
    echo "4. Verify DNS: ping $DOMAIN"
    echo ""
    echo "Copy the output above and send it for troubleshooting."
fi
